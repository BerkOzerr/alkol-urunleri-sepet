import { useDispatch, useSelector } from "react-redux";
import AlkolCard from "./AlkolCard";
import { useEffect } from "react";
import { getAllProducts } from "../lib/productSlice";
import LoginBody from "./LoginBody";

const AlkolList = () => {
  const { products, isLoading, productsSearch } = useSelector(
    (state) => state.product,
  );
  const user = useSelector((state) => state.user);

  const dispatch = useDispatch();
  useEffect(() => {
    // console.log("auth", isAuthonticate);
    // console.log("loading", isLoading);
    if (user.login_user.authonticate) {
      dispatch(getAllProducts());
    }

    //console.log(ex);
  }, [dispatch, user]);
  return (
    <>
      {user.login_user.authonticate ? (
        <>
          {isLoading ? (
            <div className="flex items-center justify-center w-full ">
              <svg
                className="flex items-center justify-center animate-spin transform-view transition-transform duration-300"
                xmlns="http://www.w3.org/2000/svg"
                height="200px"
                viewBox="0 -960 960 960"
                width="200px"
                fill="#e3e3e3"
              >
                <path d="M436.67-80q-48.67-6.33-94-24.83-45.34-18.5-86-49.17L304-202.67q31 23 64.5 36.34 33.5 13.33 68.17 19V-80Zm86.66 0v-67.33q108-19 179-100.17t71-193.83q0-123.67-84.83-208.5-84.83-84.84-208.5-84.84h-16l74 74-48 48L334.67-768 490-923.33l48 48.66-73.33 73.34H480q75 0 140.5 28.16 65.5 28.17 114.33 77 48.84 48.84 77 114.34Q840-516.33 840-441.33q0 139-89.67 240.16Q660.67-100 523.33-80Zm-328-135.33Q166-254.67 147.17-301q-18.84-46.33-25.17-97h68q5 36.67 18.33 70.83Q221.67-293 244-263.33l-48.67 48ZM122-484.67Q128.67-535 147-581t48.33-85.67L244-619.33q-22.33 31.66-35.67 65.83Q195-519.33 190-484.67h-68Z" />
              </svg>
            </div>
          ) : (
            <div className="grid w-full sm:grid-cols-1 lg:grid-cols-3 items-center justify-center gap-3">
              {productsSearch.length === 0 ? (
                <>
                  {products.map((product) => {
                    return <AlkolCard key={product.id} product={product} />;
                  })}
                </>
              ) : (
                <>
                  {productsSearch.map((product) => {
                    return <AlkolCard key={product.id} product={product} />;
                  })}
                </>
              )}
            </div>
          )}
        </>
      ) : (
        <LoginBody />
      )}
    </>
  );
};

export default AlkolList;
